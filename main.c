...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, &reading, sizeof(reading));
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  //HAL_UART_Receive(&huart1, reading, 1, 100); // Solo 2 bytes
	  	  	     //lecturaPot1 = reading[0];
	  	  	     HAL_Delay(500);
	  	  	     // montar sd
	  	  	     fres = f_mount(&fs, "/", 0);
	  	  	     if (fres == FR_OK) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
    if(huart->Instance == USART1){
        // Recibir el valor completo del potenciómetro
        lecPot1 = (reading[0] << 8) | reading[1];  // Si el valor es de 16 bits
        HAL_UART_Receive_IT(&huart1, reading, sizeof(reading));  // Reiniciar la recepción
    }
}
